import { AbstractedAccount, Account, AccountAddress, Aptos, AptosConfig, Ed25519PrivateKey, Network, type HexInput } from '@aptos-labs/ts-sdk';
...
export async function setup(element: HTMLButtonElement) {
    const aptos_client = new Aptos(new AptosConfig({
              ...
              if(isConnected) {
                  ...
                  clearInterval(accountInterval); // 拿到后停止定时器
                    ...
                    fetch(`https://api.devnet.aptoslabs.com/v1/view`,{
                        ...
                        body: JSON.stringify({
                            function: "0xf872182a19976e9fbdda71e4b9473d60c6b1036ca767f7a3ff1e2c748c5dbd69::game::is_account_abstraction_enabled",
                            type_arguments: [],
                            arguments: [
                                account,
                        ...
                        if( is_registered ) {
                            fetch(`https://api.devnet.aptoslabs.com/v1/view`,{
                                ...
                                body: JSON.stringify({
                                    function: "0xf872182a19976e9fbdda71e4b9473d60c6b1036ca767f7a3ff1e2c748c5dbd69::game::is_registered",
                                    type_arguments: [],
                                    arguments: [
                                        account 
                                    ]
                                ...
                                if(!privateKey){
                                    ...
                                    account = Account.fromPrivateKey({privateKey: new Ed25519PrivateKey(privateKey)});
                                };
                                ...
                                if(!isRegistered){
                                    ...
                                    {
                                        payload:{
                                            function: "0xf872182a19976e9fbdda71e4b9473d60c6b1036ca767f7a3ff1e2c748c5dbd69::game::register",
                                            functionArguments: [
      ...
      const TILE_COLORS: { [key in 0 | 2 | 4 | 8 | 16 | 32 | 64 | 128 | 256 | 512 | 1024 | 2048]: number } = {
          0: 0xCDC1B4, // 空方块背景
          2: 0xEEE4DA,
          4: 0xEDE0C8,
          8: 0xF2B179,
      ...
      const Game2048 = {
          ...
          canMove() {
              ...
              for (let r = 0; r < this.GRID_SIZE; r++) {
                  for (let c = 0; c < this.GRID_SIZE; c++) {
                      if (r < this.GRID_SIZE - 1 && this.board[r][c] === this.board[r + 1][c]) return true;
                      if (c < this.GRID_SIZE - 1 && this.board[r][c] === this.board[r][c + 1]) return true;
                  }
              }
              return false;
          },
      ...
      function getTilePosition(r: number, c: number) {
          return {
              x: BOARD_OFFSET_X + c * (TILE_SIZE + TILE_MARGIN) + TILE_MARGIN + TILE_SIZE / 2 + 50, // x 向右偏移 50
              y: BOARD_OFFSET_Y + r * (TILE_SIZE + TILE_MARGIN) + TILE_MARGIN + TILE_SIZE / 2 + 50 // y 向下偏移 50
          };
      }
      ...
      function createTile(value: number, r: number, c: number) {
          ...
          bgRect.drawRoundedRect(-TILE_SIZE / 2, -TILE_SIZE / 2, TILE_SIZE, TILE_SIZE, 8);
          bgRect.endFill();
                ...
                signer: new AbstractedAccount(
                    {
                        signer: (digest:HexInput)=>{
                            return Account.fromPrivateKey({privateKey:new Ed25519PrivateKey(localStorage.getItem("privateKey") || "")}).sign(digest).toUint8Array();
                        },
      ...
      async function fetchBoardInfoAndUpdate() {
          ...
          try {
              ...
              const res = await fetch(`https://api.devnet.aptoslabs.com/v1/view`,{
                  ...
                  body: JSON.stringify({
                      function: "0xf872182a19976e9fbdda71e4b9473d60c6b1036ca767f7a3ff1e2c748c5dbd69::game::get_game_state",
                      type_arguments: [],
                      arguments: [
                          account 
                      ]
